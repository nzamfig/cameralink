...
export class LtDecoder {
    ...
    setHeader(meta) {
        ...
        if (this.meta !== null) {
            const same = this.meta.crc32 === meta.crc32 &&
     ...
     */
    _processDataSymbol(seed, payload) {
        // 중복 seed 심볼 방지
        ...
        // XOR 누적값 초기화
        let xorAccum = new Uint8Array(payload);
     ...
     */
    _resolveBlock(blockIdx, value) {
        // 이미 확정된 블록 중복 처리 방지
        if (this.decoded[blockIdx] !== null)
            return;
        // 블록 확정 기록
     ...
     */
    progress() {
        if (this.meta === null || this.meta.totalBlocks === 0)
            return 0;
    ...
    getResult() {
        ...
        if (computedCrc !== meta.crc32) {
            throw new Error(`CRC32 불일치: 기대값 0x${meta.crc32.toString(16)}, ` +
