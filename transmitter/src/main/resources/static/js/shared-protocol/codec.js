...
export function encodeHeader(h) {
    ...
    view.setUint32(offset, h.originalSize, false);
    offset += 4;
    view.setUint32(offset, h.storedSize, false);
    offset += 4;
    view.setUint16(offset, h.payloadSize, false);
    offset += 2;
    view.setUint32(offset, h.totalBlocks, false);
    offset += 4;
    view.setUint32(offset, h.crc32, false);
    offset += 4;
    return buf;
}
// ─────────────────────────────────────────────
 ...
 * 총 = 10 + N 바이트
 */
export function encodeDataSymbol(d) {
 ...
 */
function decodeHeader(bytes, view) {
    ...
    offset += 4;
    const storedSize = view.getUint32(offset, false);
    offset += 4;
    const payloadSize = view.getUint16(offset, false);
    offset += 2;
    const totalBlocks = view.getUint32(offset, false);
    offset += 4;
 ...
 */
function decodeData(bytes, view) {
