 ...
 */
function rho(d: number, k: number): number {
  if (d === 1) return 1 / k;
  return 1 / (d * (d - 1));
}
 ...
 */
function tau(d: number, k: number, c: number, delta: number): number {
  const R = c * Math.sqrt(k) * Math.log(k / delta);
  const spike = Math.floor(k / R);
  if (d < spike) return R / (k * d);
  if (d === spike) return (R / k) * Math.log(R / delta);
  return 0;
}
 ...
 * @param rand Mulberry32 PRNG 함수
 */
export function sampleIndices(k: number, d: number, rand: () => number): number[] {
  const swapped = new Map<number, number>();
  const get = (i: number): number => swapped.get(i) ?? i;
  for (let i = 0; i < d; i++) {
